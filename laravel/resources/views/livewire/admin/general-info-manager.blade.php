<div>
    ...
    <section class="content"><div class="container-fluid">
        ...
        <div class="card-body">
            ...
            <div class="table-responsive"><table class="table table-striped"><thead><tr><th>Title</th><th>Category</th><th>Created</th><th></th></tr></thead><tbody>@forelse($this->infos as $info)<tr><td>{{ $info->name }}</td><td>{{ $info->metadata['category'] ?? '-' }}</td><td>{{ $info->created_at->format('Y-m-d') }}</td><td><button class="btn btn-sm btn-warning" wire:click="edit({{ $info->id }})"><i class="fas fa-edit"></i></button> <button class="btn btn-sm btn-danger" wire:click="delete({{ $info->id }})" onclick="return confirm('Delete?')"><i class="fas fa-trash"></i></button></td></tr>@empty<tr><td colspan="4" class="text-muted">No info entries.</td></tr>@endforelse</tbody></table></div>
